...
CUDA_CALLABLE inline void add_inplace(cg1t::element_p2& p, const cg1t::element_p2& q) noexcept {
  ...
  f12o::mul(t1, p.Y, q.Y);
  f12o::mul(t2, p.Z, q.Z);
  fmtgo::add(t3, p.X, p.Y);
  fmtgo::add(t4, q.X, q.Y);
  f12o::mul(t3, t3, t4);
  fmtgo::add(t4, t0, t1);
  f12o::sub(t3, t3, t4);
  fmtgo::add(t4, p.Y, p.Z);
  fmtgo::add(p.X, q.Y, q.Z);
  f12o::mul(t4, t4, p.X);
  fmtgo::add(p.X, t1, t2);
  f12o::sub(t4, t4, p.X);
  fmtgo::add(p.X, px, p.Z);
  fmtgo::add(p.Y, q.X, q.Z);
  ...
  f12o::mul(p.Y, p.Y, t0);
  f12o::mul(t1, t1, p.Z);
  fmtgo::add(p.Y, t1, p.Y);
  f12o::mul(t0, t0, t3);
  f12o::mul(p.Z, p.Z, t4);
  fmtgo::add(p.Z, p.Z, t0);
}
