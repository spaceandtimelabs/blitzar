...
TEST_CASE("test one commitments") {
  std::vector<c32t::element_p3> precomputed_values(5), generators(5);
  ...
  SECTION("we can correctly generate precomputed values") {
    REQUIRE(c32t::element_p3::identity() == precomputed_values[0]);
  ...
  SECTION("we can correctly generate one commitments at i-th position out of identity value") {
    REQUIRE(c32t::element_p3::identity() == cpu_get_one_commit(0));
  ...
  SECTION("we can correctly generate one commitments at i-th position out of a predefined "
    ...
    REQUIRE(c32t::element_p3::identity() == cpu_get_one_commit(c32t::element_p3::identity(), 0, 0));
    REQUIRE(c32t::element_p3::identity() == cpu_get_one_commit(c32t::element_p3::identity(), 0, 1));
    REQUIRE(generators[0] == cpu_get_one_commit(c32t::element_p3::identity(), 1, 0));
    REQUIRE(generators[0] == cpu_get_one_commit(generators[0], 0, 0));
    REQUIRE(generators[0] == cpu_get_one_commit(generators[0], 0, 1));
    REQUIRE(sum_gen_0_1 == cpu_get_one_commit(c32t::element_p3::identity(), 2, 0));
