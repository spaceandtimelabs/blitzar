...
TEST_CASE("we can commit to the fold of an inner product problem") {
...
TEST_CASE("we can fold an inner product problem") {
  std::pmr::monotonic_buffer_resource alloc;
  cpu_driver drv;
  proof_descriptor descriptor;
  basct::cspan<s25t::element> a_vector;
...
TEST_CASE("we can compute the expected commitment needed for verification") {
  std::pmr::monotonic_buffer_resource alloc;
  cpu_driver drv;
  proof_descriptor descriptor;
  basct::cspan<s25t::element> a_vector;
