 ...
 */
bool is_on_curve(const cgkt::element_affine& p) noexcept {
  fgkt::element y2;
  fgko::square(y2, p.Y);
  ...
  return (y2_x3 == cgkcn::b_v) || p.infinity;
}
 ...
 */
bool is_on_curve(const cgkt::element_p2& p) noexcept {
  fgkt::element y2;
  fgkt::element y2_z;
  fgko::square(y2, p.Y);
  ...
  fgko::square(z2, p.Z);
  fgko::mul(z3, z2, p.Z);
  fgko::mul(b_z3, fgkt::element{cgkcn::b_v}, z3);
