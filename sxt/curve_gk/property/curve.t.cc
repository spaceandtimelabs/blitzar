...
TEST_CASE("an affine element") {
  SECTION("equal to the generator is on the curve") {
    REQUIRE(is_on_curve(cgkcn::generator_affine_v));
  }

  SECTION("equal to the identity is on the curve") {
    REQUIRE(is_on_curve(cgkt::element_affine::identity()));
  }

  SECTION("equal to (1,1) is not on the curve") {
    constexpr cgkt::element_affine one_one{fgkcn::one_v, fgkcn::one_v, false};
...
TEST_CASE("a projective element") {
  SECTION("equal to the generator is on the curve") { REQUIRE(is_on_curve(cgkcn::generator_p2_v)); }
  ...
  SECTION("equal to the generator projected by z is on the curve") {
    ...
    constexpr fgkt::element z{cgkcn::b_v};
    fgkt::element x_projected;
    fgkt::element y_projected;
    fgko::mul(x_projected, cgkcn::generator_p2_v.X, z);
    fgko::mul(y_projected, cgkcn::generator_p2_v.Y, z);
    cgkt::element_p2 generator_projected{x_projected, y_projected, z};
  ...
  SECTION("equal to the identity is on the curve") {
    REQUIRE(is_on_curve(cgkt::element_p2::identity()));
  }

  SECTION("equal to (1,1,1) is not on the curve") {
    constexpr cgkt::element_p2 one_one_one{fgkcn::one_v, fgkcn::one_v, fgkcn::one_v};
