 ...
 */
CUDA_CALLABLE inline void subtract_p(uint32_t ret[8], const uint32_t a[8]) noexcept {
  uint32_t borrow{0};
  ...
  basfld::sbb(ret[6], borrow, a[6], p_v[6]);
  basfld::sbb(ret[7], borrow, a[7], p_v[7]);
  ...
  ret[6] = (a[6] & borrow) | (ret[6] & mask);
  ret[7] = (a[7] & borrow) | (ret[7] & mask);
}
} // namespace sxt::f32b
