...
template <class Rng>
static CUDA_CALLABLE void generate_random_element_impl(f32t::element& e, Rng& generator) noexcept {
  uint32_t data[8];
  for (int i = 0; i < 8; ++i) {
    data[i] = bast::narrow_cast<uint32_t>(generator());
  }

  bool is_below_modulus{false};
  f32b::from_bytes(is_below_modulus, e.data(), reinterpret_cast<const uint8_t*>(data));
}
