...
// unset_marker_v
//--------------------------------------------------------------------------------------------------
static constexpr uint32_t unset_marker_v = static_cast<uint32_t>(-1);
...
bool operator==(const element_p2& lhs, const element_p2& rhs) noexcept {
  ...
  f32o::mul(x1, lhs.X, rhs.Z);
  f32o::mul(x2, rhs.X, lhs.Z);

  f32o::mul(y1, lhs.Y, rhs.Z);
  f32o::mul(y2, rhs.Y, lhs.Z);

  const auto lhs_is_zero = f32p::is_zero(lhs.Z);
  const auto rhs_is_zero = f32p::is_zero(rhs.Z);
