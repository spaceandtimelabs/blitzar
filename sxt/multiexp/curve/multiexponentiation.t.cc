...
TEST_CASE("we can compute multiexponentiations") {
  auto f = [](basct::cspan<c32t::element_p3> generators,
    ...
    return compute_multiexponentiation<c32t::element_p3>(generators, exponents);
  };
...
TEST_CASE("we can compute async multiexponentiations") {
  auto f = [](basct::cspan<c32t::element_p3> generators,
              basct::cspan<mtxb::exponent_sequence> exponents) noexcept {
    auto fut = async_compute_multiexponentiation<c32t::element_p3>(generators, exponents);
    xens::get_scheduler().run();
    return memmg::managed_array<c32t::element_p3>{std::move(fut.value())};
  };
