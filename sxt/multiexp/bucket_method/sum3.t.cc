...
TEST_CASE("we accumulate buckets with a bit width of 8") {
  ...
  SECTION("we can accumulate a single element of zero") {
    ...
    auto fut = compute_bucket_sums3<E>(sums, generators, scalars, element_num_bytes, bit_width);
  ...
  SECTION("we can accumulate two elements into the same bucket") {
    ...
    scalars1[32] = 1u;
    scalars = {scalars1.data()};
    auto fut = compute_bucket_sums3<E>(sums, generators, scalars, element_num_bytes, bit_width, 1u);
    xens::get_scheduler().run();
    REQUIRE(fut.ready());
    expected[0] = 77u;
    REQUIRE(sums == expected);
  ...
  SECTION("we can accumulate multiple elements in different buckets") {
    generators = {33u, 44u};
    std::vector<uint8_t> scalars1(64);
    scalars1[0] = 1u;
    scalars1[32] = 2u;
...
TEST_CASE("we accumulate buckets with a bit width of 4") {
  unsigned element_num_bytes = 32;
  unsigned bit_width = 4;
  unsigned num_buckets_per_output = ((1u << bit_width) - 1u) * element_num_bytes;
