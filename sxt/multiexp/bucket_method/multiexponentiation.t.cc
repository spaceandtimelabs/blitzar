...
TEST_CASE("we can compute multiexponentiations with curve-21") {
  std::vector<c32t::element_p3> res(1);
  std::vector<c32t::element_p3> generators;
  std::vector<const uint8_t*> exponents;
  ...
  SECTION("we can compute a multiexponentiation with a single element of 1") {
    ...
    scalar_data[0] = 1;
    exponents.push_back(scalar_data);
    generators = {0x123_c32};
    auto fut = multiexponentiate<c32t::element_p3>(res, generators, exponents);
    xens::get_scheduler().run();
    REQUIRE(fut.ready());
    REQUIRE(res[0] == 0x123_c32);
  }
}
