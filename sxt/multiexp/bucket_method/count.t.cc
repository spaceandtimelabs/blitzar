...
TEST_CASE("we can compute the inclusive prefix sum of bucket counts") {
  ...
  std::pmr::vector<uint16_t> tile_counts(counts.size(), memr::get_managed_device_resource());

  std::pmr::vector<unsigned> expected(counts.size());
  std::pmr::vector<uint16_t> tile_expected(counts.size());

  basdv::stream stream;

  SECTION("we handle the case of a single digit of zero") {
    bytes = {0u};
    inclusive_prefix_count_buckets(counts, tile_counts, stream, bytes, element_num_bytes, bit_width,
                                   num_outputs, tile_size, 1);
    basdv::synchronize_stream(stream);
    REQUIRE(counts == expected);
    REQUIRE(tile_counts == tile_expected);
  }
  ...
  SECTION("we handle the case of a single digit of one") {
    ...
    REQUIRE(counts == expected);
    std::fill(tile_expected.begin(), tile_expected.end(), 1);
    REQUIRE(tile_counts == tile_expected);
  }
  ...
  SECTION("we handle the case of two digits of one") {
    ...
    REQUIRE(counts == expected);
    std::fill(tile_expected.begin(), tile_expected.end(), 2);
    REQUIRE(tile_counts == tile_expected);
  }
