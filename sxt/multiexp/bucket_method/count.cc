...
static __global__ void count_kernel(unsigned* __restrict__ counts,
  ...
  tile_counts += tile_index * (NumBins - 1u);
  tile_counts += (NumBins - 1u) * num_tiles * digit_index;
  tile_counts += (NumBins - 1u) * num_tiles * num_digits * output_index;
  ...
  for (unsigned i=0; i<prefix_counts_items_per_thread; ++i) {
    ...
    tile_counts[index - 1u] = prefix_sums[i];
  }


  // write bin count results
