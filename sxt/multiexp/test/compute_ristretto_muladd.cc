...
//--------------------------------------------------------------------------------------------------
void compute_ristretto_muladd(
    basct::span<rstt::compressed_element> result,
    ...
    for (size_t seq = 0; seq < result.size(); ++seq) {
      ...
      for (size_t gen_i = 0; gen_i < sequences[seq].n; ++gen_i) {
        ...
        basct::cspan<uint8_t> exponent{sequences[seq].data +
             gen_i * element_nbytes, element_nbytes};
