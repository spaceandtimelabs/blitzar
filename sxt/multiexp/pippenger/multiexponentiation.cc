...
//--------------------------------------------------------------------------------------------------
static void compute_output_digit_or_all(
    std::vector<uint8_t>& output_digit_or_all,
    basct::cspan<mtxb::exponent> output_or_all, size_t radix_log2) noexcept {
  output_digit_or_all.reserve(output_or_all.size());
  for (size_t output_index = 0; output_index < output_or_all.size();
       ++output_index) {
...
//--------------------------------------------------------------------------------------------------
static void compute_multiproduct(
  ...
  compute_output_digit_or_all(output_digit_or_all, aggregates.output_or_all,
                              radix_log2);

  make_multiproduct_table(table, exponents, aggregates.pop_count,
                          aggregates.term_or_all, output_digit_or_all,
                          radix_log2);
...
//--------------------------------------------------------------------------------------------------
void compute_multiexponentiation(
    memmg::managed_array<void>& inout, const driver& drv,
    basct::cspan<mtxb::exponent_sequence> exponents) noexcept {
  std::vector<uint8_t> output_digit_or_all;
