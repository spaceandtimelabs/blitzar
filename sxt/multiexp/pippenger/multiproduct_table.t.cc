...
TEST_CASE(
    "we can construct a table that identifies the terms needed for "
  ...
  SECTION("we correctly handle the empty case with a single input term") {
    ...
    make_multiproduct_term_table(table,
                                 {term_or_all.data(), term_or_all.size()}, 3);
...
TEST_CASE("we can construct a table for the multiproduct computation") {
  ...
  SECTION("we handle the case of zero entries") {
    ...
    make_multiproduct_table(table, sequence, 100, term_or_all,
                            output_digit_or_all, 3);
    REQUIRE(table == mtxi::index_table{});
  }
