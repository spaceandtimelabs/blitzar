...
static void verify_random_example(
    std::mt19937& rng,
  ...
  size_t num_inputs;
  size_t num_entries;
  mtxrn::generate_random_multiproduct(products, num_inputs, num_entries, rng,
                                      descriptor);
  memmg::managed_array<uint64_t> inout(num_entries);
  mtxtst::generate_uint64s(basct::span<uint64_t>{inout.data(), num_inputs},
                           rng);
