...
void prune_rows(basct::span<basct::span<uint64_t>> rows,
  ...
  for (size_t row_index = num_inactive_outputs; row_index < rows.size();
       ++row_index) {
    auto& row = rows[row_index];
    auto num_inactive_entries = row[1];
    auto num_active_entries = row.size() - 2 - num_inactive_entries;
    assert(num_active_entries > 0);
    for (size_t entry_index = 2 + num_inactive_entries;
         entry_index < row.size(); ++entry_index) {
      auto entry = row[entry_index];
      ++v1[entry];
      max_active_counts[entry] =
          std::max(max_active_counts[entry], num_active_entries);
    }
  }

  // partition the entries to separate out the isolated ones
  std::vector<uint64_t> v2(markers.size());
  auto [sep, sep_p] = std::partition_copy(
      basit::counting_iterator<uint64_t>{0},
      basit::counting_iterator<uint64_t>{markers.size()}, v2.begin(),
      v2.rbegin(), [&](size_t index) noexcept {
        assert(v1[index] > 0);
        return v1[index] == 1 || max_active_counts[index] == 1;
      });
  auto deactivation_count =
      static_cast<size_t>(std::distance(v2.begin(), sep));
  ...
  for (size_t row_index = num_inactive_outputs; row_index < rows.size();
    ...
    std::tie(sep, sep_p) = std::partition_copy(
        row.begin() + 2 + num_inactive_entries, row.end(), v2.begin(),
        v2.rbegin(),
