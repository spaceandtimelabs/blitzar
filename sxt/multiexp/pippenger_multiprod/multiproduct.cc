...
static void prune_and_permute_products(basct::span_void inout,
                          ...
                          size_t& num_inactive_outputs,
                          size_t& num_inactive_inputs, const driver& drv,
                          size_t num_active_inputs) noexcept {
  std::vector<uint64_t> permutation{
      basit::counting_iterator<uint64_t>{0},
...
void compute_multiproduct(basct::span_void inout,
  ...
  prune_and_permute_products(inout, products, num_inactive_outputs,
                             num_inactive_inputs, drv, num_inputs);
  ...
  if (params.partition_size > 0) {
    ...
    partition_inputs(inout.subspan(num_inactive_inputs), stats, products,
                     num_inactive_outputs, num_inactive_inputs, drv,
  ...
  while (num_inactive_outputs < products.size()) {
    ...
    clump_inputs(inout.subspan(num_inactive_inputs), stats, products,
                 num_inactive_outputs, num_inactive_inputs_p, drv,
                 params.input_clump_size);
    if (stats.prev_num_terms == stats.num_terms) {
      break;
    }
    num_active_inputs =
        stats.num_terms - (num_inactive_inputs_p - num_inactive_inputs);
    ...
    compute_multiproduct(inout.subspan(num_inactive_inputs),
                         clumped_output_table.header(), drv, num_active_inputs);
    rewrite_multiproducts_with_output_clumps(
        products.subspan(num_inactive_outputs), output_clumps,
        params.output_clump_size);
    num_active_inputs = output_clumps.size();
    prune_and_permute_products(inout.subspan(num_inactive_inputs), products,
                               num_inactive_outputs, num_inactive_inputs, drv,
                               num_active_inputs);
  }
