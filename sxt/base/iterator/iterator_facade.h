...
struct has_distance_to {
  static constexpr bool value = false;
};

template <class T>
struct has_distance_to<T, std::void_t<decltype(std::declval<T>().distance_to(
                              std::declval<T>()))>> {
  static constexpr bool value = true;
};

template <class T>
static constexpr bool has_distance_to_v = has_distance_to<T>::value;
}  // namespace detail
...
template <class T>
struct iterator_difference_type<T, std::enable_if_t<has_distance_to_v<T>>> {
...
struct iterator_reference<
    T, std::enable_if_t<
           std::is_same_v<typename T::reference, typename T::reference>>> {
...
struct iterator_pointer {
  using type =
      std::remove_reference_t<decltype(std::declval<T>().dereference())>*;
};

template <class T>
struct iterator_pointer<T, std::enable_if_t<std::is_same_v<
...
template <class T>
struct has_increment<T, std::void_t<decltype(std::declval<T>().increment())>> {
  static constexpr bool value = true;
};

template <class T>
static constexpr bool has_increment_v = has_increment<T>::value;
} // namespace detail
...
namespace detail {
template <class T, class = void>
struct has_decrement {
  static constexpr bool value = false;
};

template <class T>
struct has_decrement<T, std::void_t<decltype(std::declval<T>().decrement())>> {
  static constexpr bool value = true;
};

template <class T>
static constexpr bool has_decrement_v = has_decrement<T>::value;
} // namespace detail
...
template <class T>
struct has_advance<
    T, std::void_t<decltype(std::declval<T>().advance(std::ptrdiff_t{}))>> {
  static constexpr bool value = true;
};

template <class T>
static constexpr bool has_advance_v = has_advance<T>::value;
} // namespace detail
...
//--------------------------------------------------------------------------------------------------
namespace detail {
template <class T, class Delta, class = void>
struct is_valid_advance {
  static constexpr bool value = false;
};

template <class T, class Delta>
struct is_valid_advance<
    T, Delta, std::void_t<decltype(std::declval<T>().advance(Delta{}))>> {
  static constexpr bool value = true;
};
...
struct iterator_category {
  using type = std::conditional_t<
      has_advance_v<T>, std::random_access_iterator_tag,
...
struct iterator_category<
    T, std::enable_if_t<
           std::is_same_v<typename T::iterator_category, typename T::iterator_category>>> {
...
class iterator_facade {
 public:
  ...
  template <class T = Derived,
            std::enable_if_t<detail::has_increment_v<T>>* = nullptr>
  ...
  template <class T = Derived,
            std::enable_if_t<!detail::has_increment_v<T> &&
                             detail::has_advance_v<T>>* = nullptr>
  Derived& operator++() noexcept {
    this->self().advance(1);
    return self();
  }
  ...
  // operator--
  template <class T = Derived,
            std::enable_if_t<detail::has_decrement_v<T>>* = nullptr>
  Derived& operator--() noexcept {
    this->self().decrement();
    return self();
  }
  ...
  template <class T = Derived,
            std::enable_if_t<!detail::has_decrement_v<T> &&
                             detail::has_advance_v<T>>* = nullptr>
  Derived& operator--() noexcept {
    this->self().advance(-1);
    return self();
  ...
  template <
      class Delta,
      std::enable_if_t<detail::is_valid_advance_v<Derived, Delta>>* = nullptr>
 ...
 private:
  ...
  template <class T = Derived,
            std::enable_if_t<detail::has_distance_to_v<T>>* = nullptr>
