...
TEST_CASE("we can take the base 2 logarithmic of arbitrary numbers") {
  ...
  REQUIRE(log2p1(cspan<uint8_t>(
              reinterpret_cast<uint8_t*>(std::array<unsigned long long, 4>{
  ...
  REQUIRE(log2p1(cspan<uint8_t>(
              reinterpret_cast<uint8_t*>(std::array<unsigned long long, 6>{
                  17991574144297587693ull, 410974272831746224ull, 14625339942921143157ull,
                  1421653999304472111ull, 3714367641016598528ull, 3388131789017ull}
                                             .data()),
              48)) == Approx(361.62362713128294));

  // maximum allowed number
  // log2(x + 1),  x = 2^1016 - 1 (1016 bits)
  REQUIRE(log2p1(cspan<uint8_t>(reinterpret_cast<uint8_t*>(std::array<unsigned long long, 16>{
