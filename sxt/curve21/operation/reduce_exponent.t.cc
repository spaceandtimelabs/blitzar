...
TEST_CASE(
    "Test 1 - We can verify that reduce does not change input array A when A < "
    "p") {
  // p = 2^252 + 27742317777372353535851937790883648493 (decimal)
  // s = p - 1 (decimal)
  // s % p = p - 1 (decimal)
  std::array<unsigned long long, 4> s = {6346243789798364140ull,
                                         1503914060200516822ull, 0ull,
                                         1152921504606846976ull};

  SECTION(
      "Verifying for A mod p == A holds (p = 2^252 + "
...
TEST_CASE(
    "Test 2 - We can verify that reduce change input array A when A >= p") {
  // p = 2^252 + 27742317777372353535851937790883648493 (decimal)
  // s = p (decimal)
  // s % p = 0 (decimal)
  std::array<unsigned long long, 4> s = {6346243789798364141ull,
                                         1503914060200516822ull, 0ull,
                                         1152921504606846976ull};
...
TEST_CASE(
    "Test 3 - We can verify that reduce change input array A when A >= p") {
  // p = 2^252 + 27742317777372353535851937790883648493 (decimal)
  // s = p + 103 (decimal)
  // s % p = 103 (decimal)
  std::array<unsigned long long, 4> s = {6346243789798364244ull,
                                         1503914060200516822ull, 0ull,
                                         1152921504606846976ull};
...
TEST_CASE(
    "Test 4 - We can verify that reduce correctly change input array A even "
  ...
  std::array<unsigned long long, 4> expected_s = {
      15486807595281847580ull, 14334777244411350896ull, 18446744073709551614ull,
