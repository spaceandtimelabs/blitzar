...
echo "rm -rf \$BENCHMARK_NAME.pdf \$BENCHMARK_NAME.dot callgrind.out.[0-9]*" >> $OUTPUT
...
echo "$DOT -Tpdf \$BENCHMARK_NAME.dot -o \"\$BENCHMARK_NAME.pdf\"" >> $OUTPUT
echo "mv callgrind.out.[0-9]* callgrind.out.\$BENCHMARK_NAME" >> $OUTPUT
echo "zip \$BENCHMARK_NAME.zip *.pdf *.dot callgrind.out.*" >> $OUTPUT
echo "tar zcf \$BENCHMARK_NAME.tar.gz *.pdf *.dot callgrind.out.*" >> $OUTPUT
echo "rm -rf \$BENCHMARK_NAME.pdf \$BENCHMARK_NAME.dot callgrind.out.*" >> $OUTPUT
