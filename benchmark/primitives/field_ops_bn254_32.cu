...
// vector_add 
//--------------------------------------------------------------------------------------------------
void vector_add(f32t::element* vec_result, const f32t::element* vec_a, unsigned n_elements, unsigned repetitions, unsigned n_threads) {
...
// vector_mul
//--------------------------------------------------------------------------------------------------
void vector_mul(f32t::element* vec_result, const f32t::element* vec_a, unsigned n_elements, unsigned repetitions, unsigned n_threads) {
    const unsigned threads_per_block = n_threads;
    const unsigned num_blocks = basn::divide_up(n_elements, threads_per_block);
...
//--------------------------------------------------------------------------------------------------
__global__ void init_random_array_impl(f32t::element* __restrict__ rand, unsigned n_elements) {
    int tid = blockDim.x * blockIdx.x + threadIdx.x;
    if (tid < n_elements)
    {
        basn::fast_random_number_generator rng{static_cast<uint64_t>(tid + 1),
                                               static_cast<uint64_t>(n_elements + 1)};
        f32rn::generate_random_element(rand[tid], rng);
    }
}
...
// init_random_array
//--------------------------------------------------------------------------------------------------
void init_random_array(f32t::element* rand, unsigned n_elements, unsigned n_threads) {
    const unsigned threads_per_block = n_threads;
    const unsigned num_blocks = basn::divide_up(n_elements, threads_per_block);
...
// field_ops_bn254_32
//--------------------------------------------------------------------------------------------------
void field_ops_bn254_32(std::string op, unsigned n_elements, unsigned repetitions, unsigned n_threads, unsigned n_executions) noexcept {
  std::println("field_ops_bn254_32 for {}", op);

  // Allocate memory for the input and output vectors
  memmg::managed_array<f32t::element> a(n_elements, memr::get_device_resource());
  memmg::managed_array<f32t::element> ret(n_elements, memr::get_device_resource());
