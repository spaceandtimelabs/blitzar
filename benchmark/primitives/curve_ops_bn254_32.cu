...
#include "sxt/curve_bng1_32/random/element_p2.h"
...
// vector_add_impl 
//--------------------------------------------------------------------------------------------------
__global__ void vector_add_impl(cn3t::element_p2* __restrict__ vec_ret,
                                const cn3t::element_p2* __restrict__ vec_a,
                                unsigned n_elements,
                                unsigned repetitions) {
  auto tid = blockDim.x * blockIdx.x + threadIdx.x;
  if (tid < n_elements) {
    cn3t::element_p2 x = vec_a[tid];
    auto res = x;
...
// vector_add
//--------------------------------------------------------------------------------------------------
void vector_add(cn3t::element_p2* vec_result, const cn3t::element_p2* vec_a, unsigned n_elements, unsigned repetitions, unsigned n_threads) {
...
//--------------------------------------------------------------------------------------------------
__global__ void init_random_array_impl(cn3t::element_p2* __restrict__ rand, unsigned n_elements) {
    int tid = blockDim.x * blockIdx.x + threadIdx.x;
    if (tid < n_elements)
    {
        basn::fast_random_number_generator rng{static_cast<uint64_t>(tid + 1),
                                               static_cast<uint64_t>(n_elements + 1)};
        cn3rn::generate_random_element(rand[tid], rng);
    }
}
...
// init_random_array
//--------------------------------------------------------------------------------------------------
void init_random_array(cn3t::element_p2* rand, unsigned n_elements, unsigned n_threads) {
    const unsigned threads_per_block = n_threads;
    const unsigned num_blocks = basn::divide_up(n_elements, threads_per_block);
...
// curve_ops_bn254_32
//--------------------------------------------------------------------------------------------------
void curve_ops_bn254_32(unsigned n_elements, unsigned repetitions, unsigned n_threads, unsigned n_executions) noexcept {
  std::println("curve_ops_bn254_32 add");

  // Allocate memory for the input and output vectors
  memmg::managed_array<cn3t::element_p2> a(n_elements, memr::get_device_resource());
  memmg::managed_array<cn3t::element_p2> ret(n_elements, memr::get_device_resource());
