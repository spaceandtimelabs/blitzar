...
# with some allowances for Sxt-specific idioms.
#
...
# enough to handle block splitting and correctly detecting the main header subject to the Sxt
# canonical paths.
...
def reorder_headers(path):
    ...
    def file_header_filter():
        return lambda f: f.endswith('.h"') and (
            path.endswith(f[1:-3] + '.cc') or path.endswith(f[1:-3] + '.t.cc'))
